...
#add 5 future dates to final data
for(i in 1:5)
{
  max_date=max_date+1
  for(state in states)
...
#export the dataset by selecting required columns
onedayprediction=tmp3.df %>% select(confirmed_date,state_name,predicted)
#generate output without index
write.csv(onedayprediction,file="Onedayprediction.csv",row.names = FALSE)
...
tmp3.df<-data.frame(tmp3.df,predicted)
#export the dataset by selecting required columns
fivedayprediction=tmp3.df %>% select(confirmed_date,state_name,predicted)
write.csv(fivedayprediction,file="Fivedaysprediction.csv",row.names = FALSE)
